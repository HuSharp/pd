...
func createHeartbeatStream(ctx context.Context, cfg *Config) (pdpb.PDClient, pdpb.PD_RegionHeartbeatClient) {
	...
	if err != nil {
		log.Fatal("[heartbeat] create stream error", zap.Error(err))
	}
...
func (rs *Regions) handleRegionHeartbeat(wg *sync.WaitGroup, stream pdpb.PD_RegionHeartbeatClient, storeID uint64, rep report.Report) {
	...
	for _, region := range regions {
		...
		if err == io.EOF {
			log.Error("[heartbeat] receive eof error", zap.Uint64("store-id", storeID), zap.Error(err))
			err := stream.CloseSend()
			if err != nil {
				log.Error("[heartbeat] fail to close stream", zap.Uint64("store-id", storeID), zap.Error(err))
			}
			return
		}
		if err != nil {
			log.Error("[heartbeat] send result error", zap.Uint64("store-id", storeID), zap.Error(err))
			return
		}
	}
	log.Info("[heartbeat] store finish one round region heartbeat", zap.Uint64("store-id", storeID), zap.Duration("cost-time", time.Since(start)), zap.Int("reported-region-count", len(regions)))
}
...
func (rs *Regions) result(regionCount int, sec float64) {
	...
	log.Info("[heartbeat] update speed of each category", zap.String("rps", fmt.Sprintf("%.4f", float64(regionCount)/sec)),
