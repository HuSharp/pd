...
func InitCluster(ctx context.Context, cli pd.Client, httpCli pdHttp.Client) error {
	...
	log.Info("[api] init cluster info", zap.Int("total-region", totalRegion), zap.Int("total-store", totalStore), zap.Any("store-ids", storesID))
	return nil
}
...
func (c *baseCase) GetConfig() *config.CaseConfig {
	return c.cfg.Clone()
}
...
func (c *minResolvedTS) Do(ctx context.Context, cli pdHttp.Client) error {
	...
	if Debug {
		log.Info("[api] do HTTP case", zap.String("case", c.name), zap.Uint64("min-resolved-ts", minResolvedTS), zap.Any("store-min-resolved-ts", storesMinResolvedTS), zap.Error(err))
	}
	if err != nil {
		return err
	}
...
func (c *regionsStats) Do(ctx context.Context, cli pdHttp.Client) error {
	...
	if Debug {
		log.Info("[api] do HTTP case", zap.String("case", c.name), zap.Any("region-stats", regionStats), zap.Error(err))
	}
	if err != nil {
		return err
	}
