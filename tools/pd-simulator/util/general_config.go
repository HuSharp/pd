...
type GeneralConfig struct {
	...
	FlagSet    *flag.FlagSet `toml:"-" json:"-"`
	ConfigFile string        `toml:"config" json:"config"`
	PDAddrs    string        `toml:"pd-endpoints" json:"pd-endpoints"`
	Status     string        `toml:"status" json:"status"`

	Log      log.Config         `toml:"log" json:"log"`
	Logger   *zap.Logger        `toml:"-" json:"-"`
	LogProps *log.ZapProperties `toml:"-" json:"-"`
...
func NewGeneralConfig(flagSet *flag.FlagSet) *GeneralConfig {
	...
	fs.StringVar(&cfg.PDAddrs, "pd-endpoints", "127.0.0.1:2379", "pd address")
	fs.StringVar(&cfg.Log.File.Filename, "log-file", "", "log file path")
	fs.StringVar(&cfg.Status, "status", "127.0.0.1:10081", "status address")
...
func LoadTLSConfig(cfg *GeneralConfig) *tls.Config {
	...
	if err != nil {
		log.Fatal("failed to load tls config", zap.Error(err))
	}
