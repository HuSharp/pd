...
import (
	...
	config2 "github.com/tikv/pd/server/config"
	"go.uber.org/zap"
	"net/http"
	"os"
	"time"
...
func RunHTTPServer(addr string) *http.Server {
	...
	go func() {
		if err := srv.ListenAndServe(); err != nil && !e.Is(err, http.ErrServerClosed) {
...
var (
	...
	testPath   = "/Users/pingcap/CS/PingCAP/pd/tools/pd-simulator/util/example3.conf"
)

type Config struct {
	util.GeneralConfig
	API       *apicfg.Config    `toml:"api"`
	TSO       *tso.Config       `toml:"tso"`
	Heartbeat *heartbeat.Config `toml:"heartbeat"`
	Simulator *config.SimConfig `toml:"simulator"`
}
...
func DecodeConfig(origin *util.GeneralConfig, file string) Config {
	cfg := newConfig()
	var meta toml.MetaData
	var err error
	if meta, err = toml.DecodeFile(file, &cfg); err != nil {
		log.Fatal("[pd-simulator] decode config file failed", zap.Error(err))
	}
	cfg.GeneralConfig = *origin

	log.Info("[pd-simulator] decode config file", zap.String("file", file), zap.Any("config", cfg))
	...
	if cfg.Simulator.ServerConfig == nil || cfg.Simulator.ServerConfig.Name == "" {
		sc.DefaultStoreLimit = sc.StoreLimit{AddPeer: 2000, RemovePeer: 2000}

		// server config
		svrCfg := &config2.Config{
			Name:       "pd",
		...
		svrCfg.TickInterval = typeutil.NewDuration(100 * time.Millisecond)
		svrCfg.ElectionInterval = typeutil.NewDuration(3 * time.Second)
		svrCfg.LeaderPriorityCheckInterval = typeutil.NewDuration(100 * time.Millisecond)
		svrCfg.Log.Level = cfg.Simulator.ServerLogLevel
		cfg.Simulator.ServerConfig = svrCfg
		cfg.Simulator.Adjust(&meta)
		log.Info("[pd-simulator] using default config", zap.Any("config", cfg.Simulator))
	}
