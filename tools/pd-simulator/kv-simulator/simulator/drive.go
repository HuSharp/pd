...
import (
	...
	"github.com/tikv/pd/tools/pd-simulator/kv-simulator/simulator/info"
...
func (d *Driver) GetEventRunner() *EventRunner {
	return d.eventRunner
}

func (d *Driver) UpdateSimCase(name string) error {
	log.Info("[kv-simulator] update case", zap.String("name", name))
	simCase := cases.NewCase(name, d.simConfig)
	if simCase == nil {
		return errors.Errorf("failed to create case %s", name)
	}
	d.simCase = simCase
	return nil
}

func (d *Driver) GetSimCase() string {
	return d.simCase.Name
}
...
func (d *Driver) Prepare() error {
	...
	cancel()
	if err != nil {
		log.Fatal("[kv-simulator] bootstrap error", zap.Error(err))
	} else {
		log.Debug("[kv-simulator] bootstrap success")
	}
