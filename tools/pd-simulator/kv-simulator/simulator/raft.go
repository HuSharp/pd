...
import (
	...
	"github.com/tikv/pd/tools/pd-simulator/kv-simulator/simulator/simutil"
	"go.uber.org/zap"
)
...
func (r *RaftEngine) stepLeader(region *core.RegionInfo) {
	...
	if newLeader == nil {
		...
		log.Info("[kv-simulator] region has no leader", zap.Uint64("region-id", region.GetID()))
		return
	}
	log.Info("[kv-simulator] region elects a new leader",
...
func (r *RaftEngine) stepSplit(region *core.RegionInfo) {
	...
	for i := range ids {
		...
		if err != nil {
			log.Error("[kv-simulator] alloc id failed", zap.Error(err))
			return
		}
	}
	...
	if r.useTiDBEncodedKey {
		...
		if err != nil {
			log.Fatal("[kv-simulator] Generate TiDB encoded split key failed", zap.Error(err))
		}
	} else {
	...
	r.SetRegion(right)
	r.SetRegion(left)
	log.Debug("[kv-simulator] region split",
		zap.Uint64("region-id", region.GetID()),
