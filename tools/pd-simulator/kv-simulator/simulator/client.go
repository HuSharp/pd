...
func NewClient(pdAddr string, tag string) (Client, <-chan *pdpb.RegionHeartbeatResponse, error) {
	log.Info("[kv-simulator] create pd client with endpoints", zap.String("tag", tag), zap.String("pd-address", pdAddr))
	...
	log.Info("[kv-simulator] init cluster id", zap.String("tag", c.tag), zap.Uint64("cluster-id", c.clusterID))
	c.wg.Add(1)
...
func (c *client) initClusterID() error {
	...
	for i := 0; i < maxInitClusterRetries; i++ {
		...
		if err != nil || members.GetHeader() == nil {
			log.Error("[kv-simulator] failed to get cluster id", zap.String("tag", c.tag), zap.Error(err))
			continue
		}
...
func (c *client) createConn() (*grpc.ClientConn, error) {
	...
	for i := 0; i < len(urls); i++ {
		cc, err := grpc.Dial(strings.TrimPrefix(urls[i], "http://"), grpc.WithTransportCredentials(insecure.NewCredentials()))
		if err != nil {
			continue
		}
		members, err := pdpb.NewPDClient(cc).GetMembers(context.Background(), &pdpb.GetMembersRequest{})
		if err != nil {
			continue
		}
		...
		url = members.GetLeader().GetClientUrls()[0]
		break
	}
...
func (c *client) createHeartbeatStream() (pdpb.PD_RegionHeartbeatClient, context.Context, context.CancelFunc) {
	...
	for {
		...
		log.Error("[kv-simulator] create region heartbeat stream error", zap.String("tag", c.tag), zap.Error(err))
		cancel()
		select {
		case <-c.ctx.Done():
			log.Info("[kv-simulator] cancel create stream loop")
...
func (c *client) heartbeatStreamLoop() {
	...
	for {
		...
		select {
		case err := <-errCh:
			log.Error("[kv-simulator] heartbeat stream get error", zap.String("tag", c.tag), zap.Error(err))
			cancel()
		case <-c.ctx.Done():
			log.Info("[kv-simulator] cancel heartbeat stream loop")
			return
		}
		wg.Wait()
	}
...
func (c *client) reportRegionHeartbeat(ctx context.Context, stream pdpb.PD_RegionHeartbeatClient, errCh chan error, wg *sync.WaitGroup) {
	...
	for {
		...
		case r := <-c.reportRegionHeartbeatCh:
			...
			if err != nil {
				errCh <- err
				log.Error("[kv-simulator] report regionHeartbeat error", zap.String("tag", c.tag), zap.Error(err))
			}
		case <-ctx.Done():
			return
		}
...
func (c *client) Close() {
	...
	if err := c.clientConn.Close(); err != nil {
		log.Error("[kv-simulator] failed to close grpc client connection", zap.String("tag", c.tag), zap.Error(err))
	}
}
...
func (c *client) Bootstrap(ctx context.Context, store *metapb.Store, region *metapb.Region) error {
	...
	if resp.GetBootstrapped() {
		log.Fatal("[kv-simulator] failed to bootstrap, server is not clean")
	}
	if err != nil {
		return err
	}
...
func (c *client) PutStore(ctx context.Context, store *metapb.Store) error {
	...
	if resp.Header.GetError() != nil {
		log.Error("[kv-simulator] put store error", zap.Reflect("error", resp.Header.GetError()))
		return nil
	}
	return nil
}
...
func (c *client) PutPDConfig(config *config.PDConfig) error {
	if len(config.PlacementRules) > 0 {
		...
		log.Info("[kv-simulator] add placement rule success", zap.String("rules", string(content)))
	}
	if len(config.LocationLabels) > 0 {
		...
		defer res.Body.Close()
		log.Info("[kv-simulator] add location labels success", zap.String("labels", string(content)))
	}
	return nil
}
...
func (c *client) StoreHeartbeat(ctx context.Context, stats *pdpb.StoreStats) error {
	...
	if resp.Header.GetError() != nil {
		log.Error("[kv-simulator] store heartbeat error", zap.Reflect("error", resp.Header.GetError()))
		return nil
	}
	return nil
}
