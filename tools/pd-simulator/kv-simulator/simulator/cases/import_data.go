...
func newImportData(config *sc.SimConfig) *Case {
	...
	simCase.Checker = func(stores []*metapb.Store, regions *core.RegionsInfo, stats []info.StoreStats) bool {
		...
		log.Info("[kv-simulator] import data information",
		...
		if dev > 0.02 {
			log.Warn("[kv-simulator] Not balanced, change scheduler or store limit", zap.Float64("dev score", dev))
		}
...
func renderPlot(name string, data [][3]int, len, minCount, maxCount int) {
	...
	err := bar3d.Render(f)
	if err != nil {
		log.Error("[kv-simulator] render error", zap.Error(err))
	}
}
