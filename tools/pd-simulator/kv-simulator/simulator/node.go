...
func (n *Node) stepTask() {
	...
	for _, task := range n.tasks {
		if isFinished := task.Step(n.raftEngine); isFinished {
			log.Debug("[kv-simulator] task status",
...
func (n *Node) storeHeartBeat() {
	...
	if err != nil {
		log.Info("[kv-simulator] report heartbeat error",
...
func (n *Node) AddTask(task *Task) {
	...
	if t, ok := n.tasks[task.RegionID()]; ok {
		log.Debug("[kv-simulator] task has already existed",
			zap.Uint64("node-id", n.Id),
			zap.Uint64("region-id", task.RegionID()),
...
func (n *Node) Stop() {
	...
	n.client.Close()
	n.wg.Wait()
	log.Info("[kv-simulator] node stopped", zap.Uint64("node-id", n.Id))
}
