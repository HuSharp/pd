...
func (e *AddNodes) Run(raft *RaftEngine, tickCount int64) bool {
	...
	if _, ok := raft.conn.Nodes[id]; ok {
		log.Info("[kv-simulator] node has already existed", zap.Uint64("node-id", id))
		return false
	}
	...
	if err != nil {
		log.Error("[kv-simulator] add node failed", zap.Uint64("node-id", id), zap.Error(err))
		return false
	}
	...
	err = n.Start()
	if err != nil {
		log.Error("[kv-simulator] start node failed", zap.Uint64("node-id", id), zap.Error(err))
	}
	return false
}
...
func (e *DeleteNodes) Run(raft *RaftEngine, tickCount int64) bool {
	...
	if node == nil {
		log.Error("[kv-simulator] node is not existed", zap.Uint64("node-id", id))
		return false
	}
